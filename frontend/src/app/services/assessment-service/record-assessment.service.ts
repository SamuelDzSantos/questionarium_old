...
export class RecordAssessmentService {
    ...
    /** 1. Criar registros em lote */
    createBatch(request: CreateRecordAssessmentRequest): Observable<RecordAssessment[]> {
        return this.http.post<RecordAssessment[]>(this.baseUrl, request);
    }

    /** 2. Buscar registro por id */
    getById(id: number): Observable<RecordAssessment> {
        return this.http.get<RecordAssessment>(`${this.baseUrl}/${id}`);
    }

    /** 3. Listar todos os registros */
    getAll(): Observable<RecordAssessment[]> {
        return this.http.get<RecordAssessment[]>(this.baseUrl);
    }

    /** 4. Listar registros do usuário logado */
    getByUser(): Observable<RecordAssessment[]> {
        return this.http.get<RecordAssessment[]>(`${this.baseUrl}/user`);
    }
    ...
    /** 7. Consulta pública de um registro */
    getPublic(id: number): Observable<RecordAssessmentPublic> {
        return this.http.get<RecordAssessmentPublic>(`${this.baseUrl}/public/${id}`);
    }

    /** 8. PATCH: Atualiza studentAnswerKey e recalcula obtainedScore */
    patchStudentKey(id: number, patch: PatchRecordAssessmentRequest): Observable<RecordAssessment> {
        return this.http.patch<RecordAssessment>(`${this.baseUrl}/${id}`, patch);
    }
}
